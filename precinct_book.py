...
addressesFile = 'Live_Street_Address_Management_(SAM)_Addresses.geojson'
...
def parse_args():
    ...
    parser.add_argument('--addresses-file', action='store',
                        ...
                        'geojson downloaded from bdata.boston.gov')
...
def readAddresses(args):
    ...
    ranges = {}
    ids = {}
    features = geojson.load(open(args.addresses_file, 'rb'))['features']
...
def download(slug, _type, target):
    ...
    response.raise_for_status()
    with open(f'{target}.new', 'wb') as f:
